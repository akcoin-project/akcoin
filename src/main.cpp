...
#if defined(NDEBUG)
# error "Akcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf1fcda5382bb36d9b3ab34ad8689c87d118f7f38695eb45318c8bcb0e58869a8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Akcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Akcoin Signed Message:\n";
...
{
    // Akcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 350400); // Akcoin: 350k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6 * 24 * 60 * 60; // Akcoin: 6 days
static const int64 nTargetSpacing = 6 * 60; // Akcoin: 6 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Akcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Akcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd94c8592670f2e5bd70095f04fc1480141d33d5699520d7a7e7f8c4d7dac3101");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Hedef 2023! Guclu Ordu, Guclu TURKIYE";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042b320b92b7b9fb5e02931326e716d6c436ceae0980f40e3fcbb4e9d647a7d131cd5a90e6312a35288facbf412ea6abd5f67e8e18368abbd5f75c392086a439cf") << OP_CHECKSIG;
        ...
        block.nVersion = 1;
        block.nTime    = 1516662380;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084693864;

        if (fTestNet)
        {
            block.nTime    = 1516662380;
            block.nNonce   = 386188709;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5551b64e33791d69ed2487b0318747f1c55c73595bb9dc0885a90bea487939d3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe0, 0xa5, 0x9b, 0xc0 }; // Akcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AkcoinMiner(CWallet *pwallet)
{
    printf("AkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("akcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AkcoinMiner, pwallet));
}
